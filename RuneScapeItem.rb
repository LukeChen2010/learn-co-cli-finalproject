...
class RuneScapeItem
    ...
    def get_price_chart
        ...
        daily_hash.each do |key, value|
             ...
             @daily_chart[date.strftime("%B %d, %Y")] = value
        end

        average_hash.each do |key, value|
            date = Time.at(key.to_i/1000)
            @average_chart[date.strftime("%B %d, %Y")] = value
       end
    end

    def get_max_daily_value
        return @daily_chart.values.max
    end

    def get_max_daily_date
        return  @daily_chart.key(get_max_daily)
    end

    def get_max_average_value
        return @average_chart.values.max
    end

    def get_max_average_date
        return  @average_chart.key(average_chart)
    end
end